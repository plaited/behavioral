...
/** Spawn the mcp faculty PROCESS and expose the same wire harness API. */
const spawnMcpWorker = () => {
  const worker = spawnFaculty({
    file: 'mcp/faculty.ts',
    requestType: FACULTY_MESSAGE_KINDS.mcp_request,
    resultType: FACULTY_MESSAGE_KINDS.mcp_request_result,
  })
  const call = (id: string, op: string, input: unknown, space?: string): void => {
    worker.call({ id, op, input } as JsonObject, space)
  }
  const cancel = (id: string): void => {
    worker.post({ type: FACULTY_MESSAGE_KINDS.mcp_cancel, detail: { id } } as never)
  }
