...
import { MCP_CALLS_COLLECTION, MCP_EVENT_TYPES, mcpThreads } from '../threads.ts'
...
describe('mcp threads — the replay spine', () => {
  ...
  test('a completed result is never captured — no per-call store churn', () => {
    ...
    expect(selected.some((s) => s.type === FACULTY_MESSAGE_KINDS.store_request && s.detail?.op !== 'get')).toBe(false)
  })
  ...
  test('grant ingress replays the captured request and deletes the capture', () => {
    ...
    const get = selected.find((s) => s.type === FACULTY_MESSAGE_KINDS.store_request && s.detail?.op === 'get')
    expect((get?.detail?.input as JsonObject)?.key).toBe('c4')
    const retry = selected.find((s) => s.type === FACULTY_MESSAGE_KINDS.mcp_request && s.detail?.id === 'c4-retry')
    ...
    const del = selected.find(
      (s) => s.type === FACULTY_MESSAGE_KINDS.store_request && s.detail?.op === 'delete' && s.detail?.id === 'c4',
    )
  ...
  test('a store value that is not a captured request does not replay', () => {
    ...
    expect(selected.some((s) => s.type === FACULTY_MESSAGE_KINDS.mcp_request)).toBe(false)
  })
})
