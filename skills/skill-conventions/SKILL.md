...
description: >
  ...
  through threads, the shell faculty (bun run -), and the store — not fleet
  ...
  discovery threads.
license: ISC
compatibility: Requires bun and the behavioral runtime (threads + the shell and store faculties)
...
the shell faculty (`bun run -`) + the store** — there are no fleet tools for
...
Boot threads run **scan recipes** through the shell faculty; the results are
...
- validate → `{ present: [...], missing: [...] }` resolved against the
  faculty's `cwd`; with `rootRelative: true`, leading-`/` links resolve
...
//    shell faculty does not guarantee node_modules resolution in cwd) …
